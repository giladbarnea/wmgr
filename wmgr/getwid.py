...
def main(app: str, *args, **kwargs) -> str or bool:
    ...
    def print_help():
        import common
        print('\x1b[97;1mwinmgmt (global options)\x1b[0m\n', common.__doc__)
    ...
    if common.should_log:
        common.debug(f'\n⟨getwid.py⟩ {app = !r}, {args = }, {kwargs = }')

    app = str(app)  # normalize
    if common.is_wid(app):
        # ** `app` is wid, e.g. '100901473'
        if common.should_log: common.good(f'⟨getwid.py⟩ `app` is wid (8<=len<=9 and digit):')
    ...
    if common.is_hexid(app):
        ...
        if common.should_log: common.info(f'⟨getwid.py⟩ `app` is hexid (0x...) or pid (6 digits)')
    ...
    # todo: xdotool search --name --class --all pycharm is more precise than either
    # ~20ms?
    wids = []
    ...
    if not wids:
        if common.should_log: common.warn(f'⟨getwid.py⟩ xdotool search --classname did not return anything, trying with --name')
        ...
        if not wids:
            if common.should_log: common.error(f"⟨getwid.py⟩ xdotool search --onlyvisible '--classname' and '--name' returned empty for {app = !r}. Returning False")
            return False

    if common.should_log: common.info(f'⟨getwid.py⟩ {wids = !r}')
    ...
    for wid in wids:
        ...
        if same_name_means_duplicate and name in name_2_wid:
            ...
            if common.should_log: common.warn(f'\t⟨getwid.py⟩ ignoring because duplicate. {name = !r}, {wid = !r}')
            ignore.add(name)
            del name_2_wid[name]
            continue
        ...
        if common.ignore_scripts:
            ...
            if any(winmgmt_py_file in name and not (
                    # any of the conditions below mean we should not ignore
                ...
                if common.should_log: common.warn(f'\t⟨getwid.py⟩ ignoring because some winmgmt_py_file name is substring of {name = !r}, {wid = !r}, {app = !r}. {winmgmt_py_files = !r}')
                continue
        
        if should_ignore(ignore, name):
            if common.should_log: common.warn(f'\t⟨getwid.py⟩ ignoring because should_ignore({ignore}, {name}). {wid = !r}')
            continue

        name_2_wid[name] = wid

        if common.should_log: common.good(f'\t⟨getwid.py⟩ (not ignoring): {name = !r}, {wid = !r}')
    if common.should_log: common.debug(f'⟨getwid.py⟩ {name_2_wid = !r}')
    if not name_2_wid:
        if common.should_log: common.fatal("⟨getwid.py⟩: no good wid found (all ignored or none found in the first place). returning False")
        return False

    if len(name_2_wid) == 1:
        wid = name_2_wid[next(iter(name_2_wid))]
        if common.should_log: common.good(f'⟨getwid.py⟩ returning wid:')
        if common.should_output_result: print(wid, end='')
        return wid

    last = app in common.settings['last_is_most_recent']
    item = name_2_wid.popitem(last=last)
    if common.should_log: common.good(f'⟨getwid.py⟩ returning item[1]. {item = !r}')
    if common.should_output_result: print(item[1], end='')
    return item[1]
